...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>>> {
    ...
    private void performZoom(MotionEvent event) {
        ...
        if (event.getPointerCount() >= 2) {
            ...
            if (totalDist > this.mMinScalePointerDistance) {
                ...
                ViewPortHandler h2 = ((BarLineChartBase) this.mChart).getViewPortHandler();
                ...
                if (this.mTouchMode == 4) {
                    ...
                    if (isZoomingOut2) {
                        canZoomMoreX2 = h2.canZoomOutMoreX();
                    } else {
                        canZoomMoreX2 = h2.canZoomInMoreX();
                    }
                    if (isZoomingOut2) {
                        canZoomMoreY2 = h2.canZoomOutMoreY();
                    } else {
                        canZoomMoreY2 = h2.canZoomInMoreY();
                    }
                    ...
                    if (isZoomingOut) {
                        canZoomMoreX = h2.canZoomOutMoreX();
                    } else {
                        canZoomMoreX = h2.canZoomInMoreX();
                    }
                    if (canZoomMoreX) {
                        this.mMatrix.set(this.mSavedMatrix);
                    ...
                    if (isZoomingOut) {
                        canZoomMoreY = h2.canZoomOutMoreY();
                    } else {
                        canZoomMoreY = h2.canZoomInMoreY();
                    }
                    if (canZoomMoreY) {
                        this.mMatrix.set(this.mSavedMatrix);
    ...
    private void performHighlightDrag(MotionEvent e2) {
        Highlight h2 = ((BarLineChartBase) this.mChart).getHighlightByTouchPoint(e2.getX(), e2.getY());
        if (h2 != null && !h2.equalTo(this.mLastHighlighted)) {
            this.mLastHighlighted = h2;
            ((BarLineChartBase) this.mChart).highlightValue(h2, true);
        }
    }
    ...
    public boolean onDoubleTap(MotionEvent e2) {
        ...
        if (((BarLineChartBase) this.mChart).isDoubleTapToZoomEnabled() && ((BarLineScatterCandleBubbleData) ((BarLineChartBase) this.mChart).getData()).getEntryCount() > 0) {
            ...
            float f2 = 1.4f;
            float f3 = ((BarLineChartBase) this.mChart).isScaleXEnabled() ? 1.4f : 1.0f;
            if (!((BarLineChartBase) this.mChart).isScaleYEnabled()) {
                f2 = 1.0f;
            }
