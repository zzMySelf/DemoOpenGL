...
public class XAxisRendererHorizontalBarChart extends XAxisRenderer {
    ...
    public void renderAxisLabels(Canvas c2) {
        if (this.mXAxis.isEnabled() && this.mXAxis.isDrawLabelsEnabled()) {
            ...
            if (this.mXAxis.getPosition() == XAxis.XAxisPosition.TOP) {
                pointF.x = 0.0f;
                pointF.y = 0.5f;
                drawLabels(c2, this.mViewPortHandler.contentRight() + xoffset, pointF);
                ...
                pointF.x = 1.0f;
                pointF.y = 0.5f;
                drawLabels(c2, this.mViewPortHandler.contentLeft() - xoffset, pointF);
    ...
    public void drawLabels(Canvas c2, float pos, MPPointF anchor) {
        ...
        for (int i3 = 0; i3 < positions.length; i3 += 2) {
            float y = positions[i3 + 1];
            if (this.mViewPortHandler.isInBoundsY(y)) {
                drawLabel(c2, this.mXAxis.getValueFormatter().getFormattedValue(this.mXAxis.mEntries[i3 / 2], this.mXAxis), pos, y, anchor, labelRotationAngleDegrees);
            }
        }
    }
    ...
    /* access modifiers changed from: protected */
    public void drawGridLine(Canvas c2, float x, float y, Path gridLinePath) {
    ...
    public void renderLimitLines(Canvas c2) {
        ...
        if (limitLines != null && limitLines.size() > 0) {
            ...
            while (i2 < limitLines.size()) {
                LimitLine l = limitLines.get(i2);
                if (l.isEnabled()) {
                    int clipRestoreCount = c2.save();
                    this.mLimitLineClippingRect.set(this.mViewPortHandler.getContentRect());
                    this.mLimitLineClippingRect.inset(f2, -l.getLineWidth());
                    ...
                    if (label != null && !label.equals("")) {
                        ...
                        if (position == LimitLine.LimitLabelPosition.RIGHT_TOP) {
                            ...
                            canvas.drawText(label, this.mViewPortHandler.contentRight() - xOffset, (pts[c3] - yOffset) + labelLineHeight, this.mLimitLinePaint);
