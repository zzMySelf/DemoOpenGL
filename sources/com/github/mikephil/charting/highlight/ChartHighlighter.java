...
public class ChartHighlighter<T extends BarLineScatterCandleBubbleDataProvider> implements IHighlighter {
    ...
    public float getMinimumDistance(List<Highlight> closestValues, float pos, YAxis.AxisDependency axis) {
        ...
        for (int i2 = 0; i2 < closestValues.size(); i2++) {
            Highlight high = closestValues.get(i2);
    ...
    /* access modifiers changed from: protected */
    public float getHighlightPos(Highlight h2) {
        return h2.getYPx();
    }
    ...
    public List<Highlight> getHighlightsAtXValue(float xVal, float x, float y) {
        ...
        for (int i2 = 0; i2 < dataSetCount; i2++) {
            IDataSet dataSet = data.getDataSetByIndex(i2);
            if (dataSet.isHighlightEnabled()) {
                this.mHighlightBuffer.addAll(buildHighlights(dataSet, i2, xVal, DataSet.Rounding.CLOSEST));
            }
        }
        return this.mHighlightBuffer;
    }
    ...
    public List<Highlight> buildHighlights(IDataSet set, int dataSetIndex, float xVal, DataSet.Rounding rounding) {
        ...
        List<Entry> entries = iDataSet.getEntriesForXValue(f2);
        if (entries.size() == 0) {
            Entry closest = iDataSet.getEntryForXValue(f2, Float.NaN, rounding);
            if (closest != null) {
