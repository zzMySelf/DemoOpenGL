...
import com.baidu.iknow.advisory.hn.upload.AsyncUploadManager;
...
import com.baidu.swan.apps.impl.api.AdsInfoApi;
...
public class TalosFileClassifyHelper {
    ...
    private static HashMap<String, String> sMimeTypeToExtensionMap = new HashMap<>();
    ...
    static {
        ...
        add(AppDownloadBuilder.MIME_TYPE_ZIP, "zip", 8);
        ...
        add("application/msword", "doc", 4);
        add("application/msword", "dot", 4);
        add("application/vnd.openxmlformats-officedocument.wordprocessingml.document", "docx", 4);
        add("application/vnd.openxmlformats-officedocument.wordprocessingml.template", "dotx", 4);
        add("application/vnd.ms-excel", "xls", 4);
        add("application/vnd.ms-excel", "xlt", 4);
        add("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "xlsx", 4);
        add("application/vnd.openxmlformats-officedocument.spreadsheetml.template", "xltx", 4);
        add("application/vnd.ms-powerpoint", "ppt", 4);
        add("application/vnd.ms-powerpoint", "pot", 4);
        add("application/vnd.ms-powerpoint", "pps", 4);
        add("application/vnd.openxmlformats-officedocument.presentationml.presentation", "pptx", 4);
        ...
        add("application/x-director", Config.FilePath.SUBTYPE_DIR, 5);
        ...
        add("application/x-maker", AdsInfoApi.KEY_MAKER, 5);
        ...
        add(MimeType.Audio.AMR, AsyncUploadManager.AUDIO_FORMAT_AMR, 1);
        ...
        add("audio/x-pn-realaudio", Constants.KEY_RAM, 1);
        add("audio/x-realaudio", "ra", 1);
        add("audio/x-scpls", "pls", 1);
        add("audio/x-sd2", "sd2", 1);
        add(MimeType.Audio.WAV, "wav", 1);
        add("image/bmp", "bmp", 2);
        add("image/gif", "gif", 2);
        add("image/ico", "cur", 5);
        add("image/ico", "ico", 2);
        add("image/ief", "ief", 5);
        add("image/jpeg", RtcParameterSettings.VideoCodecId.JPEG, 2);
        add("image/jpeg", "jpg", 2);
        add("image/jpeg", "jpe", 2);
        add("image/pcx", "pcx", 5);
        add(MimeType.Image.PNG, CanvasToTempFileModel.IMAGE_EXT_PNG, 2);
        ...
        add("image/x-rgb", CardConstants.CARD_ITEM_LOTTERY_RGB, 5);
        ...
        add("text/css", UbcUserActionKt.CLICK_SEARCH_SERVICE, 5);
        add("text/html", "htm", 5);
        add("text/html", "html", 5);
        add("text/h323", "323", 5);
        add("text/iuls", "uls", 5);
        ...
        add("text/plain", PackageDownloadInfo.TYPE_diff, 4);
        add("text/plain", "po", 4);
        ...
        add("video/fli", "fli", 0);
        add("video/m4v", "m4v", 0);
        add(MimeType.Video.MPEG, "mpeg", 0);
        add(MimeType.Video.MPEG, "mpg", 0);
        add(MimeType.Video.MPEG, "mpe", 0);
        add("video/mp4", "mp4", 0);
        add(MimeType.Video.MPEG, "vob", 0);
    ...
    private static void add(String mimeType, String extension, int fileType) {
        sExtTypeMap.put(extension, Integer.valueOf(fileType));
    ...
    public static int getCategory(String ext, String mime) {
        return getCategory(!TextUtils.isEmpty(ext) ? ext.toLowerCase(Locale.US) : "", mime, true);
    }

    public static int getCategory(String ext, String mime, boolean isDownloadModel) {
        Integer result = sMimeTypeMap.get(mime);
        if (result == null) {
            result = sExtTypeMap.get(ext);
            if (result == null) {
                result = 5;
            } else if (isDownloadModel && result.intValue() == 8) {
                result = 5;
            }
        } else if (isDownloadModel && result.intValue() == 8) {
            result = 5;
    ...
    public static boolean isVideoKernelSupport(String extension) {
        ...
        String lowextension = extension.toLowerCase(Locale.US);
        if ("m3u".equals(lowextension) || "f4v".equals(lowextension) || "asf".equals(lowextension) || "wmv".equals(lowextension) || "avi".equals(lowextension) || BdVideoSeries.FORMAT_FLV.equals(lowextension) || "mkv".equals(lowextension) || "mov".equals(lowextension) || "mp4".equals(lowextension) || "3gp".equals(lowextension) || "3g2".equals(lowextension) || "mpg".equals(lowextension) || "mpeg".equals(lowextension) || "ts".equals(lowextension) || "rm".equals(lowextension) || "rmvb".equals(lowextension) || "webm".equals(lowextension)) {
            return true;
        }
        return false;
    }
    ...
    public static String getFileSuffix(String fileName) {
        int len;
        if (fileName == null || (len = fileName.lastIndexOf(".")) == -1 || len == fileName.length()) {
            return "";
        }
