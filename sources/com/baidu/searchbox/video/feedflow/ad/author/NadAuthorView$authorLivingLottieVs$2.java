package com.baidu.searchbox.video.feedflow.ad.author;

import android.view.ViewStub;
import com.baidu.searchbox.video.feedflow.ad.R;
import kotlin.Metadata;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.Lambda;

@Metadata(d1 = {"\u0000\n\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\u0010\u0000\u001a\n \u0002*\u0004\u0018\u00010\u00010\u0001H\n¢\u0006\u0002\b\u0003"}, d2 = {"<anonymous>", "Landroid/view/ViewStub;", "kotlin.jvm.PlatformType", "invoke"}, k = 3, mv = {1, 6, 0}, xi = 48)
/* compiled from: NadAuthorView.kt */
final class NadAuthorView$authorLivingLottieVs$2 extends Lambda implements Function0<ViewStub> {
    final /* synthetic */ NadAuthorView this$0;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    NadAuthorView$authorLivingLottieVs$2(NadAuthorView nadAuthorView) {
        super(0);
        this.this$0 = nadAuthorView;
    }

    public final ViewStub invoke() {
        return (ViewStub) this.this$0.findViewById(R.id.video_flow_nad_right_author_living_anim_vs);
    }
}
