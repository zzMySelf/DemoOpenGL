...
import com.baidu.searchbox.comment.richtext.MyLinkMovementMethod;
...
public class EllipsizeTextView extends TextView {
    ...
    private boolean isOutOfBounds(Layout layout) {
        if (((float) layout.getHeight()) > ((((float) getMeasuredHeight()) + (((float) (layout.getLineCount() - 1)) * getLineSpacingExtra())) - ((float) getPaddingBottom())) - ((float) getPaddingTop())) {
            return true;
        }
        return false;
    }
    ...
    private int computeMaxLineCount(Layout layout) {
        int availableHeight = ((getMeasuredHeight() + (((int) getLineSpacingExtra()) * (this.mMaxLines - 1))) - getPaddingTop()) - getPaddingBottom();
        for (int i2 = 0; i2 < layout.getLineCount(); i2++) {
            if (availableHeight < layout.getLineBottom(i2)) {
                return i2;
            }
        }
    ...
    public static final class Range<T extends Comparable<? super T>> {
        ...
        public Range(T lower, T upper) {
            this.mLower = lower;
            this.mUpper = upper;
            if (!CommentRuntime.GLOBAL_DEBUG || lower.compareTo(upper) <= 0) {
    ...
    public void setReservedCount(float reservedCount) {
        if (reservedCount > 0.0f) {
            this.mReservedCount = reservedCount;
        }
        requestLayout();
    }

    public boolean onTouchEvent(MotionEvent event) {
        if (MyLinkMovementMethod.getInstance().onTouch(this, event)) {
            return true;
        }
