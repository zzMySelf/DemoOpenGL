package com.baidu.searchbox.download.center.ui.video.vulcan;

import com.baidu.searchbox.player.control.element.VulcanFloatingButtonElement;
import com.baidu.searchbox.player.slot.ISlotView;
import kotlin.Metadata;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.Lambda;

@Metadata(d1 = {"\u0000\b\n\u0000\n\u0002\u0018\u0002\n\u0000\u0010\u0000\u001a\u00020\u0001H\n¢\u0006\u0002\b\u0002"}, d2 = {"<anonymous>", "Lcom/baidu/searchbox/player/slot/ISlotView;", "invoke"}, k = 3, mv = {1, 6, 0}, xi = 48)
/* compiled from: DownloadVulcanControlLayer.kt */
final class DownloadVulcanControlLayer$setupElement$3 extends Lambda implements Function0<ISlotView> {
    public static final DownloadVulcanControlLayer$setupElement$3 INSTANCE = new DownloadVulcanControlLayer$setupElement$3();

    DownloadVulcanControlLayer$setupElement$3() {
        super(0);
    }

    public final ISlotView invoke() {
        return new VulcanFloatingButtonElement();
    }
}
