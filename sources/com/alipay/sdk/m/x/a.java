package com.alipay.sdk.m.x;

import android.app.Activity;
import android.app.AlertDialog;
import android.content.Context;
import android.content.res.Resources;
import android.graphics.drawable.ColorDrawable;
import android.graphics.drawable.GradientDrawable;
import android.os.Bundle;
import android.os.Handler;
import android.os.Looper;
import android.os.Message;
import android.text.TextUtils;
import android.view.View;
import android.view.Window;
import android.view.animation.LinearInterpolator;
import android.view.animation.RotateAnimation;
import android.widget.FrameLayout;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.TextView;
import com.alipay.sdk.m.u.e;

public class a {

    /* renamed from: i  reason: collision with root package name */
    public static final String f2581i = "正在加载";

    /* renamed from: j  reason: collision with root package name */
    public static final String f2582j = "去支付宝付款";
    public static final String k = "去支付宝授权";
    public static String l = "";

    /* renamed from: a  reason: collision with root package name */
    public d f2583a;

    /* renamed from: b  reason: collision with root package name */
    public Activity f2584b;

    /* renamed from: c  reason: collision with root package name */
    public String f2585c;

    /* renamed from: d  reason: collision with root package name */
    public long f2586d = -1;

    /* renamed from: e  reason: collision with root package name */
    public final int f2587e = 1;

    /* renamed from: f  reason: collision with root package name */
    public final long f2588f = 10000;

    /* renamed from: g  reason: collision with root package name */
    public boolean f2589g = false;

    /* renamed from: h  reason: collision with root package name */
    public Handler f2590h = new c(Looper.getMainLooper());

    /* renamed from: com.alipay.sdk.m.x.a$a  reason: collision with other inner class name */
    public class C0029a implements Runnable {
        public C0029a() {
        }

        public void run() {
            if (a.this.f2583a == null) {
                a aVar = a.this;
                a aVar2 = a.this;
                d unused = aVar.f2583a = new d(aVar2.f2584b);
                a.this.f2583a.setCancelable(a.this.f2589g);
            }
            try {
                if (!a.this.f2583a.isShowing()) {
                    a.this.f2583a.show();
                    a.this.f2590h.sendEmptyMessageDelayed(1, 10000);
                }
            } catch (Exception e2) {
                e.a((Throwable) e2);
            }
        }
    }

    public class b implements Runnable {
        public b() {
        }

        public void run() {
            if (a.this.f2583a != null && a.this.f2583a.isShowing()) {
                try {
                    a.this.f2590h.removeMessages(1);
                    a.this.f2583a.dismiss();
                } catch (Exception e2) {
                    e.a((Throwable) e2);
                }
            }
        }
    }

    public class c extends Handler {
        public c(Looper looper) {
            super(looper);
        }

        public void dispatchMessage(Message message) {
            a.this.a();
        }
    }

    public a(Activity activity) {
        this.f2584b = activity;
    }

    public void b() {
        this.f2584b = null;
        this.f2583a = null;
    }

    public String c() {
        return this.f2585c;
    }

    public void d() {
        Activity activity = this.f2584b;
        if (activity != null) {
            activity.runOnUiThread(new C0029a());
        }
    }

    public void a(boolean z) {
        this.f2589g = z;
    }

    public void a(String str) {
        this.f2585c = str;
    }

    public void a() {
        Activity activity = this.f2584b;
        if (activity != null) {
            activity.runOnUiThread(new b());
        }
    }

    public class d extends AlertDialog {
        public d(Context context) {
            super(context);
        }

        private View a(Context context) {
            Context context2 = context;
            LinearLayout linearLayout = new LinearLayout(context2);
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(-2, a(context2, 50.0f));
            layoutParams.gravity = 17;
            linearLayout.setOrientation(0);
            linearLayout.setLayoutParams(layoutParams);
            GradientDrawable gradientDrawable = new GradientDrawable();
            gradientDrawable.setColor(-450944201);
            gradientDrawable.setCornerRadius((float) a(context2, 5.0f));
            linearLayout.setBackgroundDrawable(gradientDrawable);
            ImageView imageView = new ImageView(context2);
            LinearLayout.LayoutParams layoutParams2 = new LinearLayout.LayoutParams(a(context2, 20.0f), a(context2, 20.0f));
            layoutParams2.gravity = 16;
            layoutParams2.setMargins(a(a.this.f2584b, 17.0f), a(a.this.f2584b, 10.0f), a(a.this.f2584b, 8.0f), a(a.this.f2584b, 10.0f));
            imageView.setLayoutParams(layoutParams2);
            imageView.setScaleType(ImageView.ScaleType.FIT_CENTER);
            imageView.setImageDrawable(a(context2, a.l, 480));
            RotateAnimation rotateAnimation = new RotateAnimation(0.0f, 359.0f, 1, 0.5f, 1, 0.5f);
            rotateAnimation.setRepeatCount(-1);
            rotateAnimation.setDuration(900);
            rotateAnimation.setInterpolator(new LinearInterpolator());
            imageView.startAnimation(rotateAnimation);
            TextView textView = new TextView(context2);
            textView.setText(TextUtils.isEmpty(a.this.f2585c) ? a.f2581i : a.this.f2585c);
            textView.setTextSize(16.0f);
            textView.setTextColor(-1);
            LinearLayout.LayoutParams layoutParams3 = new LinearLayout.LayoutParams(-2, -2);
            layoutParams3.gravity = 16;
            layoutParams3.setMargins(0, 0, a(context2, 17.0f), 0);
            textView.setLayoutParams(layoutParams3);
            linearLayout.addView(imageView);
            linearLayout.addView(textView);
            FrameLayout frameLayout = new FrameLayout(context2);
            frameLayout.setLayoutParams(new FrameLayout.LayoutParams(-2, -2, 17));
            frameLayout.addView(linearLayout);
            return frameLayout;
        }

        public void onCreate(Bundle bundle) {
            super.onCreate(bundle);
            setContentView(a(getContext()));
            Window window = getWindow();
            if (window != null) {
                window.setBackgroundDrawable(new ColorDrawable(0));
            }
        }

        /* JADX WARNING: Removed duplicated region for block: B:18:0x003e A[SYNTHETIC, Splitter:B:18:0x003e] */
        /* JADX WARNING: Removed duplicated region for block: B:29:? A[RETURN, SYNTHETIC] */
        /* Code decompiled incorrectly, please refer to instructions dump. */
        private android.graphics.drawable.Drawable a(android.content.Context r3, java.lang.String r4, int r5) {
            /*
                r2 = this;
                r0 = 0
                java.io.ByteArrayInputStream r1 = new java.io.ByteArrayInputStream     // Catch:{ all -> 0x0037 }
                byte[] r4 = com.alipay.sdk.m.n.a.a((java.lang.String) r4)     // Catch:{ all -> 0x0037 }
                r1.<init>(r4)     // Catch:{ all -> 0x0037 }
                android.graphics.BitmapFactory$Options r4 = new android.graphics.BitmapFactory$Options     // Catch:{ all -> 0x0035 }
                r4.<init>()     // Catch:{ all -> 0x0035 }
                if (r5 > 0) goto L_0x0013
                r5 = 240(0xf0, float:3.36E-43)
            L_0x0013:
                r4.inDensity = r5     // Catch:{ all -> 0x0035 }
                android.content.res.Resources r5 = r3.getResources()     // Catch:{ all -> 0x0035 }
                android.util.DisplayMetrics r5 = r5.getDisplayMetrics()     // Catch:{ all -> 0x0035 }
                int r5 = r5.densityDpi     // Catch:{ all -> 0x0035 }
                r4.inTargetDensity = r5     // Catch:{ all -> 0x0035 }
                android.graphics.Bitmap r4 = android.graphics.BitmapFactory.decodeStream(r1, r0, r4)     // Catch:{ all -> 0x0035 }
                android.graphics.drawable.BitmapDrawable r5 = new android.graphics.drawable.BitmapDrawable     // Catch:{ all -> 0x0035 }
                android.content.res.Resources r3 = r3.getResources()     // Catch:{ all -> 0x0035 }
                r5.<init>(r3, r4)     // Catch:{ all -> 0x0035 }
                r1.close()     // Catch:{ Exception -> 0x0032 }
                goto L_0x0033
            L_0x0032:
                r3 = move-exception
            L_0x0033:
                r0 = r5
                goto L_0x0044
            L_0x0035:
                r3 = move-exception
                goto L_0x0039
            L_0x0037:
                r3 = move-exception
                r1 = r0
            L_0x0039:
                com.alipay.sdk.m.u.e.a((java.lang.Throwable) r3)     // Catch:{ all -> 0x0045 }
                if (r1 == 0) goto L_0x0043
                r1.close()     // Catch:{ Exception -> 0x0042 }
                goto L_0x0043
            L_0x0042:
                r3 = move-exception
            L_0x0043:
            L_0x0044:
                return r0
            L_0x0045:
                r3 = move-exception
                if (r1 == 0) goto L_0x004d
                r1.close()     // Catch:{ Exception -> 0x004c }
                goto L_0x004d
            L_0x004c:
                r4 = move-exception
            L_0x004d:
                throw r3
            */
            throw new UnsupportedOperationException("Method not decompiled: com.alipay.sdk.m.x.a.d.a(android.content.Context, java.lang.String, int):android.graphics.drawable.Drawable");
        }

        private int a(Context context, float f2) {
            return (int) (f2 * (context == null ? Resources.getSystem() : context.getResources()).getDisplayMetrics().density);
        }
    }

    public a(Activity activity, String str) {
        this.f2584b = activity;
        this.f2585c = str;
    }
}
