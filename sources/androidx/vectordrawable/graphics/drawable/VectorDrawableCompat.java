...
import com.baidu.searchbox.home.theme.NewHomeBgTypeUtils;
...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    private void updateStateFromTypedArray(TypedArray a2, XmlPullParser parser, Resources.Theme theme) throws XmlPullParserException {
        ...
        ColorStateList tint = TypedArrayUtils.getNamedColorStateList(a2, parser, theme, NewHomeBgTypeUtils.HOME_SKIN_TYPE_TINT, 1);
        if (tint != null) {
            state.mTint = tint;
        }
    ...
    private static class VPathRenderer {
        ...
        private void drawGroupTree(VGroup currentGroup, Matrix currentMatrix, Canvas canvas, int w, int h2, ColorFilter filter) {
            ...
            for (int i2 = 0; i2 < vGroup.mChildren.size(); i2++) {
                ...
                if (child instanceof VGroup) {
                    drawGroupTree((VGroup) child, vGroup.mStackedMatrix, canvas, w, h2, filter);
                } else if (child instanceof VPath) {
                    drawPath(currentGroup, (VPath) child, canvas, w, h2, filter);
                }
            }
            canvas.restore();
        }

        public void draw(Canvas canvas, int w, int h2, ColorFilter filter) {
            drawGroupTree(this.mRootGroup, IDENTITY_MATRIX, canvas, w, h2, filter);
        }

        private void drawPath(VGroup vGroup, VPath vPath, Canvas canvas, int w, int h2, ColorFilter filter) {
            float len;
            VPath vPath2 = vPath;
            Canvas canvas2 = canvas;
            ColorFilter colorFilter = filter;
            float scaleX = ((float) w) / this.mViewportWidth;
            float scaleY = ((float) h2) / this.mViewportHeight;
    ...
    private static class VGroup extends VObject {
        ...
        public VGroup(VGroup copy, ArrayMap<String, Object> targetsMap) {
            ...
            for (int i2 = 0; i2 < children.size(); i2++) {
                VObject vObject = children.get(i2);
        ...
        private void updateStateFromTypedArray(TypedArray a2, XmlPullParser parser) {
            ...
            this.mRotate = TypedArrayUtils.getNamedFloat(a2, parser, NadRotationPopModelKt.ROTATION, 5, this.mRotate);
        ...
        public boolean onStateChanged(int[] stateSet) {
            boolean changed = false;
            for (int i2 = 0; i2 < this.mChildren.size(); i2++) {
                changed |= this.mChildren.get(i2).onStateChanged(stateSet);
            }
            return changed;
        }
    }
    ...
    private static abstract class VPath extends VObject {
        ...
        public String nodesToString(PathParser.PathDataNode[] nodes) {
            ...
            for (int i2 = 0; i2 < nodes.length; i2++) {
                result = result + nodes[i2].mType + ":";
                float[] params = nodes[i2].mParams;
                for (int j2 = 0; j2 < params.length; j2++) {
                    result = result + params[j2] + ",";
                }
            }
            return result;
        }
    ...
    private static class VFullPath extends VPath {
        ...
        private void updateStateFromTypedArray(TypedArray a2, XmlPullParser parser, Resources.Theme theme) {
            ...
            if (TypedArrayUtils.hasAttribute(parser, "pathData")) {
                ...
                this.mStrokeWidth = TypedArrayUtils.getNamedFloat(a2, parser, ViewProps.STROKEWIDTH, 4, this.mStrokeWidth);
