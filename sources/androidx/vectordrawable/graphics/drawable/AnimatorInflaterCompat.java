...
public class AnimatorInflaterCompat {
    ...
    private static class PathDataEvaluator implements TypeEvaluator<PathParser.PathDataNode[]> {
        ...
        public PathParser.PathDataNode[] evaluate(float fraction, PathParser.PathDataNode[] startPathData, PathParser.PathDataNode[] endPathData) {
            if (PathParser.canMorph(startPathData, endPathData)) {
                ...
                for (int i2 = 0; i2 < startPathData.length; i2++) {
                    this.mNodeArray[i2].interpolatePathDataNode(startPathData[i2], endPathData[i2], fraction);
                }
    ...
    private static void setupObjectAnimator(ValueAnimator anim, TypedArray arrayObjectAnimator, int valueType, float pixelSize, XmlPullParser parser) {
        ...
        oa.setPropertyName(TypedArrayUtils.getNamedString(arrayObjectAnimator, parser, BindingXConstants.RES_KEY_PROPERTY_NAME, 0));
    }
            ...
            if (next != 3 || parser.getDepth() > depth) {
                if (type == 1) {
                    ...
                    if (name.equals("objectAnimator")) {
                        ...
                        TypedArray a2 = TypedArrayUtils.obtainAttributes(resources, theme2, attrs, AndroidResources.STYLEABLE_ANIMATOR_SET);
                        createAnimatorFromXml(context, res, theme, parser, attrs, (AnimatorSet) anim2, TypedArrayUtils.getNamedInt(a2, xmlPullParser, "ordering", 0, 0), pixelSize);
                        a2.recycle();
        ...
        while (true) {
            ...
            if (eventType == 3 || type == 1) {
                ...
                if (parser.getName().equals("propertyValuesHolder")) {
                    ...
                    String propertyName = TypedArrayUtils.getNamedString(a2, xmlPullParser, BindingXConstants.RES_KEY_PROPERTY_NAME, 3);
                    int valueType = TypedArrayUtils.getNamedInt(a2, xmlPullParser, "valueType", 2, 4);
                    int valueType2 = valueType;
                    PropertyValuesHolder pvh = loadPvh(context, res, theme, parser, propertyName, valueType);
                    if (pvh == null) {
                        pvh = getPVH(a2, valueType2, 0, 1, propertyName);
                    }
                    if (pvh != null) {
                        if (values == null) {
    ...
    private static void dumpKeyframes(Object[] keyframes, String header) {
        if (keyframes != null && keyframes.length != 0) {
            ...
            for (int i2 = 0; i2 < count; i2++) {
                ...
                Object obj = "null";
                StringBuilder append = new StringBuilder().append("Keyframe ").append(i2).append(": fraction ").append(keyframe.getFraction() < 0.0f ? obj : Float.valueOf(keyframe.getFraction())).append(", , value : ");
    ...
    private static PropertyValuesHolder loadPvh(Context context, Resources res, Resources.Theme theme, XmlPullParser parser, String propertyName, int valueType) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (next == 3 || type == 1) {
                ...
                Resources.Theme theme2 = theme;
                XmlPullParser xmlPullParser = parser;
            } else if (parser.getName().equals(SearchVideoTcUtils.TYPE_KEY_FRAME)) {
                if (valueType2 == 4) {
    ...
    private static void distributeKeyframes(Keyframe[] keyframes, float gap, int startIndex, int endIndex) {
        ...
        for (int i2 = startIndex; i2 <= endIndex; i2++) {
            keyframes[i2].setFraction(keyframes[i2 - 1].getFraction() + increment);
        }
    }
