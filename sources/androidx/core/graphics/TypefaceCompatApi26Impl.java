...
public class TypefaceCompatApi26Impl extends TypefaceCompatApi21Impl {
    ...
    public Typeface createFromFontInfo(Context context, CancellationSignal cancellationSignal, FontsContractCompat.FontInfo[] fonts, int style) {
        ...
        if (!isFontFamilyPrivateAPIAvailable()) {
            FontsContractCompat.FontInfo bestFont = findBestInfo(fontInfoArr, i3);
            try {
                pfd = context.getContentResolver().openFileDescriptor(bestFont.getUri(), TCJSON.KEY.R, cancellationSignal2);
                if (pfd == null) {
                    if (pfd != null) {
                        pfd.close();
                    }
            ...
            int i4 = 0;
            while (i4 < length) {
                FontsContractCompat.FontInfo font = fontInfoArr[i4];
