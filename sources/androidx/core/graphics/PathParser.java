...
public class PathParser {
    ...
    public static PathDataNode[] deepCopyNodes(PathDataNode[] source) {
        ...
        for (int i2 = 0; i2 < source.length; i2++) {
            copy[i2] = new PathDataNode(source[i2]);
        }
        return copy;
    }
    ...
    public static boolean canMorph(PathDataNode[] nodesFrom, PathDataNode[] nodesTo) {
        ...
        for (int i2 = 0; i2 < nodesFrom.length; i2++) {
            if (nodesFrom[i2].mType != nodesTo[i2].mType || nodesFrom[i2].mParams.length != nodesTo[i2].mParams.length) {
                return false;
            }
        }
        return true;
    ...
    public static void updateNodes(PathDataNode[] target, PathDataNode[] source) {
        for (int i2 = 0; i2 < source.length; i2++) {
            target[i2].mType = source[i2].mType;
            for (int j2 = 0; j2 < source[i2].mParams.length; j2++) {
                target[i2].mParams[j2] = source[i2].mParams[j2];
            }
        }
    }
    ...
    public static boolean interpolatePathDataNodes(PathDataNode[] target, PathDataNode[] from, PathDataNode[] to, float fraction) {
        if (target == null || from == null || to == null) {
            ...
            for (int i2 = 0; i2 < target.length; i2++) {
                target[i2].interpolatePathDataNode(from[i2], to[i2], fraction);
            }
            return true;
        }
    }
    ...
    public static class PathDataNode {
        ...
        public static void nodesToPath(PathDataNode[] node, Path path) {
            ...
            for (int i2 = 0; i2 < node.length; i2++) {
                addCommand(path, current, previousCommand, node[i2].mType, node[i2].mParams);
                previousCommand = node[i2].mType;
            }
        }
        ...
        public void interpolatePathDataNode(PathDataNode nodeFrom, PathDataNode nodeTo, float fraction) {
            ...
            while (true) {
                ...
                if (i2 < fArr.length) {
                    this.mParams[i2] = (fArr[i2] * (1.0f - fraction)) + (nodeTo.mParams[i2] * fraction);
                    i2++;
                } else {
                    return;
                }
        ...
        private static void drawArc(Path p, float x0, float y0, float x1, float y1, float a2, float b2, float theta, boolean isMoreThanHalf, boolean isPositiveArc) {
            ...
            double x0p = ((((double) f2) * cosTheta) + (((double) f3) * sinTheta)) / ((double) f6);
            double y0p = ((((double) (-f2)) * sinTheta) + (((double) f3) * cosTheta)) / ((double) f7);
            double x1p = ((((double) f4) * cosTheta) + (((double) f5) * sinTheta)) / ((double) f6);
            double y1p = ((((double) (-f4)) * sinTheta) + (((double) f5) * cosTheta)) / ((double) f7);
            double dx = x0p - x1p;
            double dy = y0p - y1p;
            ...
            if (disc < 0.0d) {
                ...
                double d2 = dsq;
                boolean z2 = z;
                drawArc(p, x0, y0, x1, y1, f6 * adjust, f7 * adjust, theta, isMoreThanHalf, isPositiveArc);
                return;
            }
            boolean z3 = z;
            ...
            double d5 = eta1;
            double cx2 = cx * ((double) f6);
            double cy2 = ((double) f7) * cy;
            double cy3 = (cx2 * sinTheta) + (cy2 * cosTheta);
            double d6 = cy3;
            arcToBezier(p, (cx2 * cosTheta) - (cy2 * sinTheta), cy3, (double) f6, (double) f7, (double) f2, (double) f3, thetaD, eta0, sweep);
        }
        ...
        private static void arcToBezier(Path p, double cx, double cy, double a2, double b2, double e1x, double e1y, double theta, double start, double sweep) {
            ...
            double ep1y = ((-d2) * sinTheta * sinEta1) + (b2 * cosTheta * cosEta1);
