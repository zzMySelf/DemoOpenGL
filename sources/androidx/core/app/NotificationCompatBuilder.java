...
class NotificationCompatBuilder implements NotificationBuilderWithBuilderAccessor {
    ...
    NotificationCompatBuilder(NotificationCompat.Builder b2) {
        List<String> people;
        List<String> people2;
        this.mBuilderCompat = b2;
        this.mContext = b2.mContext;
        if (Build.VERSION.SDK_INT >= 26) {
            this.mBuilder = new Notification.Builder(b2.mContext, b2.mChannelId);
        } else {
            this.mBuilder = new Notification.Builder(b2.mContext);
        }
        Notification n = b2.mNotification;
        this.mBuilder.setWhen(n.when).setSmallIcon(n.icon, n.iconLevel).setContent(n.contentView).setTicker(n.tickerText, b2.mTickerView).setVibrate(n.vibrate).setLights(n.ledARGB, n.ledOnMS, n.ledOffMS).setOngoing((n.flags & 2) != 0).setOnlyAlertOnce((n.flags & 8) != 0).setAutoCancel((n.flags & 16) != 0).setDefaults(n.defaults).setContentTitle(b2.mContentTitle).setContentText(b2.mContentText).setContentInfo(b2.mContentInfo).setContentIntent(b2.mContentIntent).setDeleteIntent(n.deleteIntent).setFullScreenIntent(b2.mFullScreenIntent, (n.flags & 128) != 0).setLargeIcon(b2.mLargeIcon).setNumber(b2.mNumber).setProgress(b2.mProgressMax, b2.mProgress, b2.mProgressIndeterminate);
        if (Build.VERSION.SDK_INT < 21) {
            this.mBuilder.setSound(n.sound, n.audioStreamType);
        }
        if (Build.VERSION.SDK_INT >= 16) {
            this.mBuilder.setSubText(b2.mSubText).setUsesChronometer(b2.mUseChronometer).setPriority(b2.mPriority);
            Iterator<NotificationCompat.Action> it = b2.mActions.iterator();
            while (it.hasNext()) {
                addAction(it.next());
            }
            if (b2.mExtras != null) {
                this.mExtras.putAll(b2.mExtras);
            }
            if (Build.VERSION.SDK_INT < 20) {
                if (b2.mLocalOnly) {
                    this.mExtras.putBoolean(NotificationCompatExtras.EXTRA_LOCAL_ONLY, true);
                }
                if (b2.mGroupKey != null) {
                    this.mExtras.putString(NotificationCompatExtras.EXTRA_GROUP_KEY, b2.mGroupKey);
                    if (b2.mGroupSummary) {
                ...
                if (b2.mSortKey != null) {
                    this.mExtras.putString(NotificationCompatExtras.EXTRA_SORT_KEY, b2.mSortKey);
                }
            }
            this.mContentView = b2.mContentView;
            this.mBigContentView = b2.mBigContentView;
        }
        if (Build.VERSION.SDK_INT >= 17) {
            this.mBuilder.setShowWhen(b2.mShowWhen);
        }
        if (Build.VERSION.SDK_INT >= 19 && Build.VERSION.SDK_INT < 21 && (people2 = combineLists(getPeople(b2.mPersonList), b2.mPeople)) != null && !people2.isEmpty()) {
            this.mExtras.putStringArray(NotificationCompat.EXTRA_PEOPLE, (String[]) people2.toArray(new String[people2.size()]));
        }
        if (Build.VERSION.SDK_INT >= 20) {
            this.mBuilder.setLocalOnly(b2.mLocalOnly).setGroup(b2.mGroupKey).setGroupSummary(b2.mGroupSummary).setSortKey(b2.mSortKey);
            this.mGroupAlertBehavior = b2.mGroupAlertBehavior;
        }
        if (Build.VERSION.SDK_INT >= 21) {
            this.mBuilder.setCategory(b2.mCategory).setColor(b2.mColor).setVisibility(b2.mVisibility).setPublicVersion(b2.mPublicVersion).setSound(n.sound, n.audioAttributes);
            if (Build.VERSION.SDK_INT < 28) {
                people = combineLists(getPeople(b2.mPersonList), b2.mPeople);
            } else {
                people = b2.mPeople;
            }
            ...
            this.mHeadsUpContentView = b2.mHeadsUpContentView;
            if (b2.mInvisibleActions.size() > 0) {
                Bundle carExtenderBundle = b2.getExtras().getBundle("android.car.EXTENSIONS");
                ...
                for (int i2 = 0; i2 < b2.mInvisibleActions.size(); i2++) {
                    listBundle.putBundle(Integer.toString(i2), NotificationCompatJellybean.getBundleForAction(b2.mInvisibleActions.get(i2)));
                }
                carExtenderBundle.putBundle("invisible_actions", listBundle);
                extenderBundleCopy.putBundle("invisible_actions", listBundle);
                b2.getExtras().putBundle("android.car.EXTENSIONS", carExtenderBundle);
                this.mExtras.putBundle("android.car.EXTENSIONS", extenderBundleCopy);
            }
        }
        if (Build.VERSION.SDK_INT >= 23 && b2.mSmallIcon != null) {
            this.mBuilder.setSmallIcon(b2.mSmallIcon);
        }
        if (Build.VERSION.SDK_INT >= 24) {
            this.mBuilder.setExtras(b2.mExtras).setRemoteInputHistory(b2.mRemoteInputHistory);
            if (b2.mContentView != null) {
                this.mBuilder.setCustomContentView(b2.mContentView);
            }
            if (b2.mBigContentView != null) {
                this.mBuilder.setCustomBigContentView(b2.mBigContentView);
            }
            if (b2.mHeadsUpContentView != null) {
                this.mBuilder.setCustomHeadsUpContentView(b2.mHeadsUpContentView);
            }
        }
        if (Build.VERSION.SDK_INT >= 26) {
            this.mBuilder.setBadgeIconType(b2.mBadgeIcon).setSettingsText(b2.mSettingsText).setShortcutId(b2.mShortcutId).setTimeoutAfter(b2.mTimeout).setGroupAlertBehavior(b2.mGroupAlertBehavior);
            if (b2.mColorizedSet) {
                this.mBuilder.setColorized(b2.mColorized);
            }
            if (!TextUtils.isEmpty(b2.mChannelId)) {
                this.mBuilder.setSound((Uri) null).setDefaults(0).setLights(0, 0, 0).setVibrate((long[]) null);
            }
        }
        if (Build.VERSION.SDK_INT >= 28) {
            Iterator<Person> it2 = b2.mPersonList.iterator();
            while (it2.hasNext()) {
                this.mBuilder.addPerson(it2.next().toAndroidPerson());
            }
        }
        if (Build.VERSION.SDK_INT >= 29) {
            this.mBuilder.setAllowSystemGeneratedContextualActions(b2.mAllowSystemGeneratedContextualActions);
            this.mBuilder.setBubbleMetadata(NotificationCompat.BubbleMetadata.toPlatform(b2.mBubbleMetadata));
            if (b2.mLocusId != null) {
                this.mBuilder.setLocusId(b2.mLocusId.toLocusId());
            }
        }
        if (b2.mSilent) {
