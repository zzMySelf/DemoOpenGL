...
public class ViewPager extends ViewGroup {
    ...
    private ItemInfo infoForCurrentScrollPosition() {
        ...
        while (i4 < this.mItems.size()) {
            ...
            if (scrollX < f4 || i4 == this.mItems.size() - 1) {
                return itemInfo2;
            }
    ...
    public void smoothScrollTo(int i2, int i3, int i4) {
        ...
        float f = (float) clientWidth;
        float f2 = (float) i10;
        float distanceInfluenceForSnapDuration = f2 + (distanceInfluenceForSnapDuration(Math.min(1.0f, (((float) Math.abs(i8)) * 1.0f) / f)) * f2);
        int abs = Math.abs(i4);
        if (abs > 0) {
            i6 = Math.round(Math.abs(distanceInfluenceForSnapDuration / ((float) abs)) * 1000.0f) * 4;
        } else {
            i6 = (int) (((((float) Math.abs(i8)) / ((f * this.mAdapter.getPageWidth(this.mCurItem)) + ((float) this.mPageMargin))) + 1.0f) * 100.0f);
        }
