...
public class SwitchCompat extends CompoundButton {
    ...
    private boolean hitThumb(float f, float f2) {
        ...
        if (f <= ((float) i5) || f >= ((float) i6) || f2 <= ((float) i4) || f2 >= ((float) i7)) {
            return false;
        }
        return true;
    }
    ...
    public void onMeasure(int i2, int i3) {
        ...
        this.mThumbWidth = Math.max(this.mShowText ? Math.max(this.mOnLayout.getWidth(), this.mOffLayout.getWidth()) + (this.mThumbTextPadding * 2) : 0, i5);
        Drawable drawable2 = this.mTrackDrawable;
        if (drawable2 != null) {
