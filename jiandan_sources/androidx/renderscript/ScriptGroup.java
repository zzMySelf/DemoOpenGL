...
public final class ScriptGroup extends BaseObj {
    ...
    public Object[] execute(Object... objArr) {
        if (objArr.length < this.mInputs2.size()) {
            toString() + " receives " + objArr.length + " inputs, less than expected " + this.mInputs2.size();
            return null;
        }
        if (objArr.length > this.mInputs2.size()) {
            toString() + " receives " + objArr.length + " inputs, more than expected " + this.mInputs2.size();
        }
        int i2 = 0;
        for (int i3 = 0; i3 < this.mInputs2.size(); i3++) {
            Object obj = objArr[i3];
            if ((obj instanceof Future) || (obj instanceof Input)) {
                toString() + ": input " + i3 + " is a future or unbound value";
                return null;
            }
