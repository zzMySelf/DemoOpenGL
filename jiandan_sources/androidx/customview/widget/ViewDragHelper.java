...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int i2, int i3, int i4) {
        ...
        float distanceInfluenceForSnapDuration = f + (distanceInfluenceForSnapDuration(Math.min(1.0f, ((float) Math.abs(i2)) / ((float) width))) * f);
        int abs = Math.abs(i3);
        if (abs > 0) {
    ...
    private int computeSettleDuration(View view, int i2, int i3, int i4, int i5) {
        ...
        float f6 = f4 / f3;
        return (int) ((((float) computeAxisDuration(i2, clampMag, this.mCallback.getViewHorizontalDragRange(view))) * f5) + (((float) computeAxisDuration(i3, clampMag2, this.mCallback.getViewVerticalDragRange(view))) * f6));
    }
