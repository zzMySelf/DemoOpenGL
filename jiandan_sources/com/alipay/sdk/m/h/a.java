package com.alipay.sdk.m.h;

import com.alipay.sdk.m.g.c;

public class a extends com.alipay.sdk.m.g.a {
    public static final /* synthetic */ boolean d = (!a.class.desiredAssertionStatus());

    public a(byte[] bArr) {
        super(bArr);
    }

    public static a a(String str, long j, b bVar, short s, e eVar) throws Exception {
        String str2 = str;
        b bVar2 = bVar;
        e eVar2 = eVar;
        byte[] a = c.a((byte) 1);
        if (d || a.length == 1) {
            byte[] a2 = c.a(str2.charAt(0), str2.charAt(1));
            if (d || a2.length == 2) {
                byte[] a3 = c.a(j);
                if (d || a3.length == 8) {
                    byte[] b = c.b();
                    if (d || b.length == 2) {
                        bVar.a();
                        byte[] a4 = c.a(bVar2.a);
                        if (d || a4.length == 1) {
                            byte[] a5 = c.a(bVar2.b);
                            if (d || a5.length == 1) {
                                byte[] bArr = (byte[]) bVar2.c.clone();
                                if (d || bArr.length == (bVar2.b & 255)) {
                                    byte[] a6 = c.a(s);
                                    if (d || a6.length == 2) {
                                        byte[] b2 = c.b();
                                        if (d || b2.length == 2) {
                                            eVar.a();
                                            byte[] a7 = c.a(eVar2.a);
                                            if (d || a7.length == 1) {
                                                byte[] bArr2 = (byte[]) eVar2.b.clone();
                                                if (d || bArr2.length == (eVar2.a & 255)) {
                                                    byte[] c = c.c();
                                                    if (d || c.length == 4) {
                                                        return new a(c.a(a, a2, a3, b, a4, a5, bArr, a6, b2, a7, bArr2, c));
                                                    }
                                                    throw new AssertionError();
                                                }
                                                throw new AssertionError();
                                            }
                                            throw new AssertionError();
                                        }
                                        throw new AssertionError();
                                    }
                                    throw new AssertionError();
                                }
                                throw new AssertionError();
                            }
                            throw new AssertionError();
                        }
                        throw new AssertionError();
                    }
                    throw new AssertionError();
                }
                throw new AssertionError();
            }
            throw new AssertionError();
        }
        throw new AssertionError();
    }

    public static a c() {
        try {
            return a(com.alipay.sdk.m.g.a.c, 0, new c(""), 0, new f());
        } catch (Exception unused) {
            return null;
        }
    }
}
