...
public final class InsertDelta<T> extends AbstractDelta<T> {
    ...
    public void applyTo(@NotNull List<T> list, @NotNull ListUpdateCallback<?> listUpdateCallback) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            list.add(position + i2, lines.get(i2));
        }
        listUpdateCallback.qw(position, getTarget().getPosition(), lines.size());
    }
