package com.otaliastudios.opengl.program;

import fe.vvv.ad.de.ad;
import fe.vvv.ad.yj.qw;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.Lambda;

@Metadata(bv = {1, 0, 3}, d1 = {"\u0000\b\n\u0000\n\u0002\u0010\u0002\n\u0000\u0010\u0000\u001a\u00020\u0001H\n¢\u0006\u0002\b\u0002"}, d2 = {"<anonymous>", "", "invoke"}, k = 3, mv = {1, 4, 0}, pn = "", xi = 0, xs = "")
public final class GlProgram$draw$1 extends Lambda implements Function0<Unit> {
    public final /* synthetic */ ad $drawable;
    public final /* synthetic */ float[] $modelViewProjectionMatrix;
    public final /* synthetic */ qw this$0;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    public GlProgram$draw$1(qw qwVar, ad adVar, float[] fArr) {
        super(0);
        this.this$0 = qwVar;
        this.$drawable = adVar;
        this.$modelViewProjectionMatrix = fArr;
    }

    public final void invoke() {
        this.this$0.uk(this.$drawable, this.$modelViewProjectionMatrix);
        this.this$0.th(this.$drawable);
        this.this$0.yj(this.$drawable);
    }
}
