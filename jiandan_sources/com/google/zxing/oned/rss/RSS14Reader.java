...
public final class RSS14Reader extends AbstractRSSReader {
    ...
    private DataCharacter decodeDataCharacter(BitArray bitArray, FinderPattern finderPattern, boolean z) throws NotFoundException {
        ...
        for (int i6 = 0; i6 < dataCharacterCounters.length; i6++) {
            float f = ((float) dataCharacterCounters[i6]) / sum;
            int i7 = (int) (0.5f + f);
            if (i7 <= 0) {
                i7 = 1;
            } else if (i7 > 8) {
            ...
            if ((i6 & 1) == 0) {
                oddCounts[i8] = i7;
                oddRoundingErrors[i8] = f - ((float) i7);
            } else {
                evenCounts[i8] = i7;
                evenRoundingErrors[i8] = f - ((float) i7);
            }
        }
    ...
    private Pair decodePair(BitArray bitArray, boolean z, int i2, Map<DecodeHintType, ?> map) {
        ...
        try {
            ...
            if (resultPointCallback != null) {
                ...
                resultPointCallback.foundPossibleResultPoint(new ResultPoint(f, (float) i2));
            }
