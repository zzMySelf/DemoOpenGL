...
public final class CodaBarReader extends OneDReader {
    ...
    private void validatePattern(int i2) throws NotFoundException {
        ...
        for (int i9 = 0; i9 < 2; i9++) {
            fArr2[i9] = 0.0f;
            int i10 = i9 + 2;
            fArr2[i10] = ((((float) iArr[i9]) / ((float) iArr2[i9])) + (((float) iArr[i10]) / ((float) iArr2[i10]))) / 2.0f;
            fArr[i9] = fArr2[i10];
            fArr[i10] = ((((float) iArr[i10]) * 2.0f) + 1.5f) / ((float) iArr2[i10]);
        }
        loop3:
        while (true) {
            ...
            while (i12 >= 0) {
                ...
                float f = (float) this.counters[i2 + i12];
                if (f >= fArr2[i13] && f <= fArr[i13]) {
                    i11 >>= 1;
                    i12--;
                }
            }
