package com.google.android.gms.common.internal;

import android.util.Log;
import com.google.android.gms.common.annotation.KeepForSdk;

@KeepForSdk
public final class GmsLogger {
    public static final int zzev = 15;
    public static final String zzew = null;
    public final String zzex;
    public final String zzey;

    public GmsLogger(String str, String str2) {
        Preconditions.checkNotNull(str, "log tag cannot be null");
        Preconditions.checkArgument(str.length() <= 23, "tag \"%s\" is longer than the %d character maximum", str, 23);
        this.zzex = str;
        if (str2 == null || str2.length() <= 0) {
            this.zzey = null;
        } else {
            this.zzey = str2;
        }
    }

    private final String zza(String str, Object... objArr) {
        String format = String.format(str, objArr);
        String str2 = this.zzey;
        if (str2 == null) {
            return format;
        }
        return str2.concat(format);
    }

    private final String zzh(String str) {
        String str2 = this.zzey;
        if (str2 == null) {
            return str;
        }
        return str2.concat(str);
    }

    @KeepForSdk
    public final boolean canLog(int i2) {
        return Log.isLoggable(this.zzex, i2);
    }

    @KeepForSdk
    public final boolean canLogPii() {
        return false;
    }

    @KeepForSdk
    public final void d(String str, String str2) {
        if (canLog(3)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void e(String str, String str2) {
        if (canLog(6)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void efmt(String str, String str2, Object... objArr) {
        if (canLog(6)) {
            zza(str2, objArr);
        }
    }

    @KeepForSdk
    public final void i(String str, String str2) {
        if (canLog(4)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void pii(String str, String str2) {
        if (canLogPii()) {
            String valueOf = String.valueOf(str);
            if (" PII_LOG".length() != 0) {
                valueOf.concat(" PII_LOG");
            } else {
                new String(valueOf);
            }
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void v(String str, String str2) {
        if (canLog(2)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void w(String str, String str2) {
        if (canLog(5)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void wfmt(String str, String str2, Object... objArr) {
        if (canLog(5)) {
            zza(str2, objArr);
        }
    }

    @KeepForSdk
    public final void wtf(String str, String str2, Throwable th2) {
        if (canLog(7)) {
            zzh(str2);
            Log.wtf(str, zzh(str2), th2);
        }
    }

    @KeepForSdk
    public final void d(String str, String str2, Throwable th2) {
        if (canLog(3)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void e(String str, String str2, Throwable th2) {
        if (canLog(6)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void i(String str, String str2, Throwable th2) {
        if (canLog(4)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void pii(String str, String str2, Throwable th2) {
        if (canLogPii()) {
            String valueOf = String.valueOf(str);
            if (" PII_LOG".length() != 0) {
                valueOf.concat(" PII_LOG");
            } else {
                new String(valueOf);
            }
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void v(String str, String str2, Throwable th2) {
        if (canLog(2)) {
            zzh(str2);
        }
    }

    @KeepForSdk
    public final void w(String str, String str2, Throwable th2) {
        if (canLog(5)) {
            zzh(str2);
        }
    }

    public GmsLogger(String str) {
        this(str, (String) null);
    }
}
