        ...
        if (checkVibratePermission() && (vibrator = this.mVibrator) != null && (jArr = this.timings) != null && jArr.length > 0 && vibrator.hasVibrator()) {
            if (Build.VERSION.SDK_INT >= 26) {
                ...
                if (iArr == null || iArr.length <= 0) {
                    ...
                    if (jArr2.length == 1) {
                        this.mVibrator.vibrate(VibrationEffect.createOneShot(jArr2[0], -1));
                    } else {
                        this.mVibrator.vibrate(VibrationEffect.createWaveform(jArr2, -1));
                    }
                } else {
                    long[] jArr3 = this.timings;
                    if (jArr3.length == iArr.length) {
                        if (jArr3.length == 1) {
                            this.mVibrator.vibrate(VibrationEffect.createOneShot(jArr3[0], iArr[0]));
                        } else {
                            this.mVibrator.vibrate(VibrationEffect.createWaveform(jArr3, iArr, -1));
                        }
