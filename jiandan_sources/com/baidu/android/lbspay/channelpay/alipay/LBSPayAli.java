...
public class LBSPayAli implements NoProguard {
    ...
    public static class a {
        public static LBSPayAli a = new LBSPayAli();
    }
    ...
    private void handleError(String str) {
        LBSOriginalPayBackManage.getInstance().originalCallbackResult(PayChannelController.ALIPAY_PAYCHANNEL, "2", str);
        if (ALI_AUTH_PAY.equals(this.mServiceType)) {
            IChannelPay iChannelPay = this.mChannelPay;
            if (iChannelPay != null) {
                iChannelPay.payCancel();
            } else {
                LBSPayResult.payResult((Context) null, 2, "");
            }
    ...
    public boolean finishAuthPay(Intent intent) {
        ...
        if (ALI_AUTH_PAY.equals(this.mServiceType)) {
            if (this.mChannelPay == null) {
                handleError(data.toString());
                return false;
            } else if (AUTHPAY_SUCCESS_HOST.equals(host)) {
                if (!ExifInterface.GPS_DIRECTION_TRUE.equals(data.getQueryParameter("is_success")) || !"TRADE_SUCCESS".equals(data.getQueryParameter("trade_status"))) {
                    LBSOriginalPayBackManage.getInstance().originalCallbackResult(PayChannelController.ALIPAY_PAYCHANNEL, "2", data.toString());
                    this.mChannelPay.payError(Result.RESULT_FAILED, PayCallBackManager.PAY_FAIL_MSG);
                } else {
                    LBSOriginalPayBackManage.getInstance().originalCallbackResult(PayChannelController.ALIPAY_PAYCHANNEL, "0", data.toString());
            ...
            if (AUTHPAY_SUCCESS_HOST.equals(host)) {
                if (ExifInterface.GPS_DIRECTION_TRUE.equals(data.getQueryParameter("is_success"))) {
                    LBSOriginalPayBackManage.getInstance().originalCallbackResult(PayChannelController.ALIPAY_PAYCHANNEL, "0", data.toString());
                    LBSPayResult.payResult((Context) null, 0, data.getQuery());
                } else {
                    LBSOriginalPayBackManage.getInstance().originalCallbackResult(PayChannelController.ALIPAY_PAYCHANNEL, "2", data.toString());
                    LBSPayResult.payResult((Context) null, 2, data.getQuery());
                }
            } else if (AUTHPAY_CANCEL_HOST.equals(host)) {
                LBSOriginalPayBackManage.getInstance().originalCallbackResult(PayChannelController.ALIPAY_PAYCHANNEL, "2", data.toString());
                LBSPayResult.payResult((Context) null, 2, "");
