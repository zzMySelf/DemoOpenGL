...
public class Crypto {
    ...
    public static byte[] aesDecrypt(byte[] bArr, String str) {
        ...
        try {
            ...
            instance.init(2, a2, new IvParameterSpec(copyOfRange));
            int length = copyOf.length + copyOfRange.length;
            return instance.doFinal(bArr, length, bArr.length - length);
        } catch (Exception e2) {
            e2.printStackTrace();
            return null;
        }
    ...
    public static byte[] aesEncrypt(byte[] bArr, String str) {
        ...
        try {
            Cipher instance = Cipher.getInstance("AES/CTR/NoPadding");
            instance.init(1, a3);
            return a(a2, instance.getIV(), instance.doFinal(bArr));
        } catch (Exception e2) {
            e2.printStackTrace();
            return null;
        }
    ...
    public static byte[] desDecrypt(byte[] bArr, String str) throws Exception {
        ...
        try {
            Cipher instance = Cipher.getInstance("DES/CTR/NoPadding");
            instance.init(2, b2, new IvParameterSpec(copyOfRange));
            int length = copyOf.length + copyOfRange.length;
            return instance.doFinal(bArr, length, bArr.length - length);
        } catch (Exception e2) {
            e2.printStackTrace();
    ...
    public static SecretKey a(String str, byte[] bArr) {
        ...
        try {
            return new SecretKeySpec(SecretKeyFactory.getInstance(a()).generateSecret(new PBEKeySpec(str.toCharArray(), bArr, 1000, 128)).getEncoded(), AESUtil.ALGORITHM_NAME);
        } catch (Exception e2) {
            e2.printStackTrace();
            return null;
        }
